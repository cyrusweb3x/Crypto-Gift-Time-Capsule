...
export default function HomePage() {
  ...
  const fetchGiftCount = useCallback(async () => {
    try {
      ...
      const total = Number(giftCounter) + Number(rpCounter);
      setGiftsSent(total.toString());
    } catch {
      setGiftsSent("0");
    }
  }, []);
  ...
  useEffect(() => {
    fetchGiftCount();
    const interval = setInterval(fetchGiftCount, 60000);
    ...
    <div className="min-h-screen bg-background pb-24 font-sans text-foreground">
      ...
      <main className="mx-auto max-w-[480px] px-6 py-8">
        ...
        <section className="mb-12 flex flex-col items-center text-center">
          ...
          >
            Gift Time <br />
            <span className="text-primary">Base Capsule</span>
          </motion.h1>
          <motion.p
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
        ...
        {/* ── Transparency ── */}
        <section className="mb-8 rounded-3xl bg-secondary/50 p-6">
        ...
        <motion.section
          ...
          className="mt-10 rounded-3xl border border-border bg-secondary/30 px-6 py-6 text-center"
        >
          <p className="mb-1 text-xs font-bold uppercase tracking-widest text-muted-foreground">
            Feedback & Support
          </p>
          <p className="mb-3 text-sm font-medium text-foreground leading-relaxed">
            If you have any complaints about the app, face any issues while using it,
            or would like to share your thoughts — we'd love to hear from you.
          </p>
          <a
            href="https://forms.gle/Kj9voqVDRHgmLN8aA"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1.5 text-sm font-bold text-primary underline underline-offset-4 hover:opacity-75 transition-opacity"
          >
